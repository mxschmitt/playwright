...
import { DownloadParams, downloadBrowser } from './downloader';
...
async function downloadBrowserWithProgressBarOutOfProcess(title: string, browserDirectory: string, url: string, zipPath: string, executablePath: string | undefined, connectionTimeout: number): Promise<{ error: Error | null }> {
  ...
  const downloadParams: DownloadParams = {
    ...
    log: message => debugLogger.log('install', message),
    progress: (downloadedBytes: number, totalBytes: number) => progress(downloadedBytes, totalBytes),
  };

  try {
    await downloadBrowser(downloadParams);
  } catch (error) {
    return { error };
  }
  return { error: null };
