...
const printExpectedArgs = (expected) => expected.length === 0
    ? NO_ARGUMENTS
    : expected.map(arg => (0, jest_matcher_utils_1.printExpected)(arg)).join(', ');
const printReceivedArgs = (received, expected) => received.length === 0
    ...
    : received
        ...
        ? printCommon(arg)
        : (0, jest_matcher_utils_1.printReceived)(arg))
        .join(', ');
const printCommon = (val) => (0, jest_matcher_utils_1.DIM_COLOR)((0, jest_matcher_utils_1.stringify)(val));
const isEqualValue = (expected, received) => (0, expect_utils_1.equals)(expected, received, [...(0, jestMatchersObject_1.getCustomEqualityTesters)(), expect_utils_1.iterableEquality]);
...
const printNumberOfReturns = (countReturns, countCalls) => `\nNumber of returns: ${(0, jest_matcher_utils_1.printReceived)(countReturns)}${countCalls !== countReturns
    ? `\nNumber of calls:   ${(0, jest_matcher_utils_1.printReceived)(countCalls)}`
    : ''}`;
...
const printExpectedReceivedCallsPositive = (expected, indexedCalls, expand, isOnlyCall, iExpectedCall) => {
    ...
    if (isOnlyCall && (iExpectedCall === 0 || iExpectedCall === undefined)) {
        ...
        if (isLineDiffableCall(expected, received)) {
            ...
            const lines = [
                (0, jest_matcher_utils_1.EXPECTED_COLOR)('- Expected'),
            ...
            for (let i = 0; i < length; i += 1) {
                if (i < expected.length && i < received.length) {
                    ...
                    if (isLineDiffableArg(expected[i], received[i])) {
                        const difference = (0, jest_matcher_utils_1.diff)(expected[i], received[i], { expand });
                ...
                if (i < expected.length)
                    lines.push(`${(0, jest_matcher_utils_1.EXPECTED_COLOR)(`- ${(0, jest_matcher_utils_1.stringify)(expected[i])}`)},`);
                if (i < received.length)
                    lines.push(`${(0, jest_matcher_utils_1.RECEIVED_COLOR)(`+ ${(0, jest_matcher_utils_1.stringify)(received[i])}`)},`);
            }
            return `${lines.join('\n')}\n`;
        }
...
const printDiffCall = (expected, received, expand) => received
    ...
    if (i < expected.length) {
        ...
        if (isLineDiffableArg(expected[i], arg)) {
            const difference = (0, jest_matcher_utils_1.diff)(expected[i], arg, { expand });
            if (typeof difference === 'string' &&
                difference.includes('- Expected') &&
                difference.includes('+ Received')) {
    ...
    return `${indentation +
        (i < expected.length
            ? `  ${(0, jest_matcher_utils_1.printReceived)(arg)}`
            : (0, jest_matcher_utils_1.RECEIVED_COLOR)(`+ ${(0, jest_matcher_utils_1.stringify)(arg)}`))},`;
})
    .join('\n');
...
const isLineDiffableArg = (expected, received) => {
    ...
    const receivedType = (0, jest_get_type_1.getType)(received);
    if (expectedType !== receivedType)
        return false;
    if ((0, jest_get_type_1.isPrimitive)(expected))
        return false;
...
const printResult = (result, expected) => result.type === 'throw'
    ...
    : result.type === 'incomplete'
        ...
        : isEqualValue(expected, result.value)
            ...
            : (0, jest_matcher_utils_1.printReceived)(result.value);
...
const createToBeCalledMatcher = (matcherName) => function (received, expected) {
    ...
    const pass = count > 0;
    const message = pass
        ? () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of calls: ${(0, jest_matcher_utils_1.printExpected)(0)}\n` +
            `Received number of calls: ${(0, jest_matcher_utils_1.printReceived)(count)}\n\n` +
            calls
                .reduce((lines, args, i) => {
        ...
        : () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of calls: >= ${(0, jest_matcher_utils_1.printExpected)(1)}\n` +
            `Received number of calls:    ${(0, jest_matcher_utils_1.printReceived)(count)}`;
    return { message, pass };
};
const createToReturnMatcher = (matcherName) => function (received, expected) {
    ...
    const message = pass
        ? () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of returns: ${(0, jest_matcher_utils_1.printExpected)(0)}\n` +
            `Received number of returns: ${(0, jest_matcher_utils_1.printReceived)(count)}\n\n` +
            received.mock.results
                .reduce((lines, result, i) => {
                if (result.type === 'return' && lines.length < PRINT_LIMIT)
                    lines.push(`${i + 1}: ${(0, jest_matcher_utils_1.printReceived)(result.value)}`);
                return lines;
            }, [])
                .join('\n') +
            (received.mock.calls.length !== count
                ? `\n\nReceived number of calls:   ${(0, jest_matcher_utils_1.printReceived)(received.mock.calls.length)}`
                : '')
        : () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of returns: >= ${(0, jest_matcher_utils_1.printExpected)(1)}\n` +
            `Received number of returns:    ${(0, jest_matcher_utils_1.printReceived)(count)}` +
            (received.mock.calls.length !== count
                ? `\nReceived number of calls:      ${(0, jest_matcher_utils_1.printReceived)(received.mock.calls.length)}`
                : '');
...
const createToBeCalledTimesMatcher = (matcherName) => function (received, expected) {
    ...
    const message = pass
        ? () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of calls: not ${(0, jest_matcher_utils_1.printExpected)(expected)}`
        : () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of calls: ${(0, jest_matcher_utils_1.printExpected)(expected)}\n` +
            `Received number of calls: ${(0, jest_matcher_utils_1.printReceived)(count)}`;
    return { message, pass };
};
const createToReturnTimesMatcher = (matcherName) => function (received, expected) {
    ...
    const message = pass
        ? () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of returns: not ${(0, jest_matcher_utils_1.printExpected)(expected)}` +
            (received.mock.calls.length !== count
                ? `\n\nReceived number of calls:       ${(0, jest_matcher_utils_1.printReceived)(received.mock.calls.length)}`
                : '')
        : () => (0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
            '\n\n' +
            `Expected number of returns: ${(0, jest_matcher_utils_1.printExpected)(expected)}\n` +
            `Received number of returns: ${(0, jest_matcher_utils_1.printReceived)(count)}` +
            (received.mock.calls.length !== count
                ? `\nReceived number of calls:   ${(0, jest_matcher_utils_1.printReceived)(received.mock.calls.length)}`
                : '');
...
const createToBeCalledWithMatcher = (matcherName) => function (received, ...expected) {
    ...
    const message = pass
        ? () => {
            ...
            return ((0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
                '\n\n' +
                `Expected: not ${printExpectedArgs(expected)}\n` +
                (calls.length === 1 && (0, jest_matcher_utils_1.stringify)(calls[0]) === (0, jest_matcher_utils_1.stringify)(expected)
                    ? ''
                    : printReceivedCallsNegative(expected, indexedCalls, calls.length === 1)) +
                `\nNumber of calls: ${(0, jest_matcher_utils_1.printReceived)(calls.length)}`);
        }
        : () => {
        ...
        ? () => {
            ...
            return ((0, jest_matcher_utils_1.matcherHint)(matcherName, receivedName, expectedArgument, options) +
                ...
                (results.length === 1 &&
                    ...
                    (0, jest_matcher_utils_1.stringify)(results[0].value) === (0, jest_matcher_utils_1.stringify)(expected)
                    ? ''
...
const createNthCalledWithMatcher = (matcherName) => function (received, nth, ...expected) {
    ...
    if (!Number.isSafeInteger(nth) || nth < 1) {
        throw new Error((0, jest_matcher_utils_1.matcherErrorMessage)((0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, expectedArgument, options), `${expectedArgument} must be a positive integer`, (0, jest_matcher_utils_1.printWithType)(expectedArgument, nth, jest_matcher_utils_1.stringify)));
    }
    const receivedIsSpy = isSpy(received);
    const receivedName = receivedIsSpy ? 'spy' : received.getMockName();
    const calls = receivedIsSpy
...
const ensureMockOrSpy = (received, matcherName, expectedArgument, options) => {
    if (!isMock(received) && !isSpy(received)) {
        throw new Error((0, jest_matcher_utils_1.matcherErrorMessage)((0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, expectedArgument, options), `${(0, jest_matcher_utils_1.RECEIVED_COLOR)('received')} value must be a mock or spy function`, (0, jest_matcher_utils_1.printWithType)('Received', received, jest_matcher_utils_1.printReceived)));
    }
};
const ensureMock = (received, matcherName, expectedArgument, options) => {
    if (!isMock(received)) {
        throw new Error((0, jest_matcher_utils_1.matcherErrorMessage)((0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, expectedArgument, options), `${(0, jest_matcher_utils_1.RECEIVED_COLOR)('received')} value must be a mock function`, (0, jest_matcher_utils_1.printWithType)('Received', received, jest_matcher_utils_1.printReceived)));
    }
};
