...
const createMatcher = (matcherName, fromPromise) => function (received, expected) {
    ...
    let thrown = null;
    if (fromPromise && (0, expect_utils_1.isError)(received)) {
        ...
        if (typeof received !== 'function') {
            if (!fromPromise) {
                ...
                throw new Error((0, jest_matcher_utils_1.matcherErrorMessage)((0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, placeholder, options), `${(0, jest_matcher_utils_1.RECEIVED_COLOR)('received')} value must be a function`, (0, jest_matcher_utils_1.printWithType)('Received', received, jest_matcher_utils_1.printReceived)));
            }
        }
        else {
            try {
    ...
    if (expected === undefined) {
        ...
        throw new Error((0, jest_matcher_utils_1.matcherErrorMessage)((0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, undefined, options), `${(0, jest_matcher_utils_1.EXPECTED_COLOR)('expected')} value must be a string or regular expression or class or error`, (0, jest_matcher_utils_1.printWithType)('Expected', expected, jest_matcher_utils_1.printExpected)));
    }
};
exports.createMatcher = createMatcher;
const matchers = {
    toThrow: (0, exports.createMatcher)('toThrow'),
    toThrowError: (0, exports.createMatcher)('toThrowError'),
};
    ...
    const message = pass
        ...
        : () => (0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, undefined, options) +
            ...
            (thrown === null
                ...
                : thrown.hasMessage
                    ?
                        (0, jest_matcher_utils_1.printDiffOrStringify)(expectedMessageAndCause, thrownMessageAndCause, `Expected ${messageAndCause(expected)}`, `Received ${messageAndCause(thrown.value)}`, true) +
                            '\n' +
                            formatStack(thrown)
    ...
    const message = pass
        ? () => (0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, undefined, options) +
            ...
            (thrown !== null &&
                ...
                ? (0, print_1.printReceivedConstructorNameNot)('Received constructor', thrown.value.constructor, expected)
                : '') +
            '\n' +
            (thrown !== null && thrown.hasMessage
                ? formatReceived('Received message: ', thrown, 'message') +
        ...
        : () => (0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, undefined, options) +
            ...
            (thrown === null
                ...
                : `${thrown.value != null &&
                    typeof thrown.value.constructor === 'function'
                    ? (0, print_1.printReceivedConstructorName)('Received constructor', thrown.value.constructor)
                    : ''}\n${thrown.hasMessage
                    ? formatReceived('Received message: ', thrown, 'message') +
                        formatStack(thrown)
                    : formatReceived('Received value: ', thrown, 'value')}`);
...
const toThrow = (matcherName, options, thrown) => {
    const pass = thrown !== null;
    const message = pass
        ? () => (0, jest_matcher_utils_1.matcherHint)(matcherName, undefined, '', options) +
            '\n\n' +
            (thrown !== null && thrown.hasMessage
...
const formatExpected = (label, expected) => `${label + (0, jest_matcher_utils_1.printExpected)(expected)}\n`;
const formatReceived = (label, thrown, key, expected) => {
    ...
    if (key === 'message') {
        ...
        if (typeof expected === 'string') {
            ...
            if (index !== -1) {
                return `${label +
                    (0, print_1.printReceivedStringContainExpectedSubstring)(message, index, expected.length)}\n`;
            }
        }
        else if (expected instanceof RegExp) {
            return `${label +
                (0, print_1.printReceivedStringContainExpectedResult)(message, typeof expected.exec === 'function' ? expected.exec(message) : null)}\n`;
        }
        return `${label + (0, jest_matcher_utils_1.printReceived)(message)}\n`;
    }
    if (key === 'name') {
        return thrown.isError
            ? `${label + (0, jest_matcher_utils_1.printReceived)(thrown.value.name)}\n`
            : '';
    }
    if (key === 'value')
        return thrown.isError ? '' : `${label + (0, jest_matcher_utils_1.printReceived)(thrown.value)}\n`;
    return '';
};
const formatStack = (thrown) => thrown === null || !thrown.isError
    ? ''
    : (0, jest_message_util_1.formatStackTrace)((0, jest_message_util_1.separateMessageFromStack)(thrown.value.stack).stack, {
        rootDir: process.cwd(),
        testMatch: [],
    }, {
        noStackTrace: false,
