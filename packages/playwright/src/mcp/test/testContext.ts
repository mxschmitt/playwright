...
export class TestContext {
  ...
  async runTestsWithGlobalSetupAndPossiblePause(params: RunTestsParams, signal?: AbortSignal): Promise<{ output: string, status: FullResultStatus | 'paused' }> {
    // Serialize test runs so an aborted run fully stops + tears down before a new run starts.
    const runPromise = this._queue.then(() => this._innerRunTests(params, signal));
    this._queue = runPromise.then(() => undefined, () => undefined);
