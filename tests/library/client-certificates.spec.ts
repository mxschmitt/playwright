...
test.describe('fetch', () => {
  ...
  test('should keep supporting http', async ({ playwright, server }) => {
    const request = await playwright.request.newContext({
      ca: [path.join(kClientCertificatesDir, 'server/server_cert.pem')],
      clientCertificates: [{
        url: server.PREFIX,
        certs: [{
  ...
  test('should work with a matching CA', async ({ playwright, serverURL }) => {
    const request = await playwright.request.newContext({
      ca: [path.join(kClientCertificatesDir, 'server/server_cert.pem')],
      clientCertificates: [{
        url: serverURL,
  ...
  test('should throw with a untrusted CA', async ({ playwright, serverURL }) => {
    const request = await playwright.request.newContext({
      ca: [path.join(kClientCertificatesDir, 'server/server_cert2.pem')],
      clientCertificates: [{
        url: serverURL,
        certs: [{
          cert: path.join(kClientCertificatesDir, 'client/alice_cert.pem'),
...
test.describe('browser', () => {
  ...
  test('should fail with no client certificates', async ({ browser, serverURL }) => {
    const page = await browser.newPage({
      ca: [path.join(kClientCertificatesDir, 'server/server_cert.pem')],
      clientCertificates: [{
  ...
  test('should fail with untrusted client certs', async ({ browser, serverURL }) => {
    const page = await browser.newPage({
      ca: [path.join(kClientCertificatesDir, 'server/server_cert.pem')],
      clientCertificates: [{
        url: serverURL,
  ...
  test('should pass with matching certificates', async ({ browser, serverURL }) => {
    const page = await browser.newPage({
      ca: [path.join(kClientCertificatesDir, 'server/server_cert.pem')],
      clientCertificates: [{
        url: serverURL,
